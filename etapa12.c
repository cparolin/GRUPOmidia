...
int verificar_pagina(Simulador *sim, int pid, int pagina) {
    ...
    if (!proc || pagina >= proc->num_paginas) {
...
int carregar_pagina(Simulador *sim, int pid, int pagina) {
    ...
    // If no free frame, use the selected replacement algorithm
    if (frame == -1) {
        ...
        } else if (sim->algoritmo == 3) { // Random
...
int traduzir_endereco(Simulador *sim, int pid, int endereco_virtual) {
    ...
    // Construct the physical address: frame number * page size + offset
...
void executar_simulacao(Simulador *sim, int algoritmo) {
    ...
    // Define a sequence of memory accesses (based on the example in the document)
    int acessos[][2] = {
        {1, 1111}, 
        {1, 4444}, 
        {2, 2000}, 
